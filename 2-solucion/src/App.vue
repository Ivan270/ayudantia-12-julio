...
export default {
  ...
  methods: {
    ...
    sendMessageHandler(id, message, firstName, LastName) {
      ...
      // Para que scroll baje siempre al ultimo mensaje
      nextTick(() => {
        // nextTick permite ejecutar codigo despues de haber cambiado datos y Vue haya actualizado el DOM virtual, pero antes de que el navegador muestre esos cambios en la pagina. Es una utilidad para esperar la siguiente actualización del DOM.
        // Cuando se aplican cambios en un estado reactivo de Vue (en este caso los mensajes), Los cambios resultantes del DOM no son aplicados de forma sincrónica. Sino que Vue los hace esperar hasta el 'next tick' para asegurarse de que cada componente se actualice una sola vez sin importar cuantos cambios de estado se hayan hecho.
        const element = this.$refs.chatBoxRef.lastElementChild;
        console.log(element)

        // && como evaluacion de cortocirtuito. Si es que el operando izquierdo es falsy, el operador AND se detiene y no ejecuta la expresion de la derecha.
...
#chatBox {
  min-height: 50vh;
  /* con overflow-y, solo aparecerá barra de scroll cuando sea necesario */
  overflow-y: auto;
  max-height: 50vh;
}
.card {
